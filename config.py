...
def handle_subprocess(cmd: list[str]) -> Optional[str]:
    process_result: CompletedProcess = subprocess.run(cmd, capture_output=True)
...
while True:
    ...
    if (main_ip_match := re.match(r".*?/Net/0/.*?value:\s+(?P<IP>\d+\.\d+\.\d+\.\d+)", vbox_ip_stdout)) and main_ip_match.group("IP"):
        ...
        with ssh_config_path.open(mode="r+") as ssh_config_file:
            ...
            if ((vm_config_slice_match := vm_config_pattern.match(ssh_config))
                ...
                vm_config_slice_updated: str = re.sub(r"Hostname\s+\S+", f"Hostname {vm_ip}", vm_config_slice)
                # ssh_config_update_test: str = re.sub(vm_config_pattern, vm_config_slice_updated, ssh_config)
